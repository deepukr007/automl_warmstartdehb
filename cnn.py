...
class torchModel(nn.Module):
    ...
    def __init__(self, config, input_shape=(1, 28, 28), num_classes=10):
        ...
        key_channels_conv = 'n_channels_conv_'
        key_kernel_conv = 'kernel_conv_'
        out_channels = config.get(key_channels_conv + '0')
        ...
        self.standardize = config["standardize"] if "standardize" in config else False

        for i in range(n_conv_layers):
            kernel_size = config.get(key_kernel_conv + str(i))*2+1 if key_kernel_conv + str(i) in config else (config['kernel_size'] if 'kernel_size' in config else 5)
            ...
            out_channels_tmp = config.get(key_channels_conv + str((i + 1)))
